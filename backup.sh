...
LOG_FILE="/var/log/backup_123pan.log"
...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "201" ] || [ "$HTTP_CODE" = "204" ]; then
    ...
    ls -t /tmp/workspace-backup-*.tar.gz | tail -n +4 | xargs -r rm -f
    echo "已清理本地临时文件，保留最新3个" >> "$LOG_FILE"
    echo "云端保留所有历史备份（空间充足）" >> "$LOG_FILE"

    # 列出当前云端备份
    echo "当前云端备份:" >> "$LOG_FILE"
    curl -s -u "${WEBDAV_USER}:${WEBDAV_PASS}" \
      "${WEBDAV_URL}/备份/" | grep -o "href=\"[^\"]*workspace-backup" | sed 's/href="//;s/"//' >> "$LOG_FILE" 2>&1
