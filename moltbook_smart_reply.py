...
def post_reply(post_id, reply_content):
    ...
    try:
        ...
        endpoints = [
            ...
            f"https://www.moltbook.com/api/v1/replies"
        ]
        ...
        for endpoint in endpoints:
            response = requests.post(
                ...
                json={"post_id": post_id, "content": reply_content},
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                # Method not allowed, 尝试下一个endpoint
                continue
            else:
                # 其他错误，记录但继续尝试
                continue

        return False, "所有API endpoint都失败"
...
def main():
    ...
    for post in posts:
        ...
        # 6. 发布回复
        success, result = post_reply(post_id, reply)
        ...
        if success:
            ...
            log(f"❌ 回复发布失败: {result}")
            log("提示: 可能API endpoint不正确，已记录帖子供参考")
